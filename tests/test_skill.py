...
from falsiq.attacks import ReviewCandidate, ReviewCandidateBatch, build_selection_envelope
...
GUARD = ROOT / "skill" / "scripts" / "guard_open_reviews.py"
REQUIRE_CLI = ROOT / "skill" / "scripts" / "require_cli.sh"
REVIEWERS = REVIEW_CLASSES
...
def candidate(case_id: str, klass: str, *, score: int = 1) -> ReviewCandidate:
    ...
    return ReviewCandidate(
        ...
        risk_scenario=f"The {klass} behavior loses user data.",
...
def write_batches(directory: Path, case_id: str, *, empty: bool = False) -> list[Path]:
    ...
    for index, reviewer in enumerate(REVIEWERS):
        batch = ReviewCandidateBatch(
            case_id=case_id,
            reviewer=reviewer,
            candidates=[] if empty else [candidate(case_id, reviewer, score=index + 1)],
        )
...
def test_assemble_round_is_input_order_independent_and_machine_verified(tmp_path: Path) -> None:
    ...
    expected = build_selection_envelope(
        case_id,
        1,
        [candidate(case_id, reviewer, score=index + 1) for index, reviewer in enumerate(REVIEWERS)],
    )
...
def test_assemble_round_requires_exactly_one_batch_per_reviewer(tmp_path: Path) -> None:
    case_id = make_id(1)
    paths = write_batches(tmp_path, case_id)
    ...
    with pytest.raises(AssemblyError, match="duplicate reviewer"):
        assemble_attack_round(case_id, 1, [*paths[:-1], paths[0]])
    assert missing.returncode == 2
    assert "exactly five" in missing.stderr
    assert duplicate.returncode == 2
    assert "duplicate reviewer" in duplicate.stderr


@pytest.mark.parametrize("reviewer", REVIEWERS)
def test_review_request_carries_the_exact_schema_and_valid_examples(
    tmp_path: Path,
    reviewer: str,
) -> None:
    ...
    request = build_review_request(ledger, case_id, reviewer)

    assert request.reviewer == reviewer
    assert request.response_schema == ReviewCandidateBatch.model_json_schema()
    assert "attack" not in json.dumps(request.response_schema).casefold()
    if reviewer == "conflict":
    ...
    parsed_examples = [ReviewCandidateBatch.model_validate(example) for example in request.examples]
    ...
    assert all(example.reviewer == reviewer for example in parsed_examples)
    assert (
        ReviewGenerationRequest.model_validate_json(canonical_review_request_json(request))
        == request
    )
...
@pytest.mark.parametrize(
    ...
    [
        ...
        b'"reviewer":"boundary","candidates":[],"commentary":"extra"}',
    ],
)
def test_prepare_review_batch_replaces_invalid_agent_output_with_an_empty_batch(
    tmp_path: Path,
    payload: bytes,
) -> None:
    path = tmp_path / "reviewer-output.json"
    path.write_bytes(payload)
    case_id = make_id(1)

    batch, degraded = prepare_review_batch(case_id, "boundary", path)

    assert degraded is True
    assert batch == ReviewCandidateBatch(
        case_id=case_id,
        reviewer="boundary",
        candidates=[],
    )
...
def test_prepare_review_batch_keeps_valid_agent_output(tmp_path: Path) -> None:
    case_id = make_id(1)
    expected = ReviewCandidateBatch(
        case_id=case_id,
        reviewer="boundary",
...
@pytest.mark.parametrize("wrong_field", ["case_id", "reviewer"])
def test_prepare_review_batch_rejects_valid_schema_for_the_wrong_request_identity(
    tmp_path: Path,
    wrong_field: str,
) -> None:
    case_id = make_id(1)
    payload = ReviewCandidateBatch(
        case_id=make_id(2) if wrong_field == "case_id" else case_id,
        reviewer="omission" if wrong_field == "reviewer" else "boundary",
        candidates=[],
    )
    ...
    assert batch.reviewer == "boundary"
    assert batch.candidates == []


def test_prepare_review_batch_contains_unsafe_response_files(tmp_path: Path) -> None:
    case_id = make_id(1)
    ...
    symlink_batch, symlink_degraded = prepare_review_batch(case_id, "boundary", symlink)
...
def test_assemble_round_rejects_case_mismatch_and_symlink_input(tmp_path: Path) -> None:
    case_id = make_id(1)
    paths = write_batches(tmp_path, case_id)
    mismatched = ReviewCandidateBatch(case_id=make_id(2), reviewer="boundary", candidates=[])
...
def test_guard_blocks_open_attacks_and_missing_derivation(tmp_path: Path) -> None:
    ...
    with pytest.raises(GuardError, match="1 open review"):
        ready_brief(case_id, repo)
    assert opened.returncode == 2
    assert "1 open review" in opened.stderr
...
def test_skill_contract_and_scripted_transcript_encode_human_barriers() -> None:
    ...
    required_skill_phrases = (
        ...
        "falsiq==0.1.0",
        "falsiq review assemble",
        "falsiq review request",
        "falsiq review prepare",
    ...
    forbidden = transcript.index("$ falsiq rule <REVIEW> forbidden")
    round_two_agents = transcript.index(
        "[five fresh class-specific reviewers run in parallel for round 2]"
    )
    round_two_assembly = transcript.index("$ falsiq review assemble --case <CASE> --round 2")
...
def test_skill_activation_is_explicit_only_and_agent_vocabulary_is_neutral() -> None:
    skill = (ROOT / "skill" / "SKILL.md").read_text(encoding="utf-8")
    transcript = (ROOT / "skill" / "fixtures" / "workflow_transcript.md").read_text(
        encoding="utf-8"
    )
    production_prompts = "\n".join(
        path.read_text(encoding="utf-8") for path in sorted((ROOT / "falsiq/prompts").glob("*.md"))
    )
    normalized_skill = " ".join(skill.casefold().split())

    assert "with falsiq" in normalized_skill
    assert "presence of `.falsiq/` never activates" in normalized_skill
    assert "Use automatically" not in skill
    assert "when a .falsiq/ directory exists" not in skill
    assert "attacker" not in skill.casefold()
    assert "attacker" not in transcript.casefold()
    assert "attacker" not in production_prompts.casefold()


def test_production_prompts_have_one_packaged_source_of_truth() -> None:
    skill = (ROOT / "skill" / "SKILL.md").read_text(encoding="utf-8")
    prompt_dir = ROOT / "falsiq" / "prompts"
    prompt_names = tuple(f"reviewer_{reviewer}.md" for reviewer in REVIEWERS) + ("deriver.md",)
...
def test_installed_console_workflow_is_portable_to_a_repo_without_project_files(
    ...
    requested_reviewer = run_installed_cli(
        "review",
        "request",
        "--case",
        case_id,
    ...
    assert requested_reviewer.returncode == 0, requested_reviewer.stderr
    reviewer_request = json.loads(requested_reviewer.stdout)
    assert reviewer_request["response_schema"] == ReviewCandidateBatch.model_json_schema()
    assert "attack" not in requested_reviewer.stdout.casefold()

    malformed_path = tmp_path / "malformed-reviewer.json"
    malformed_path.write_text("not json", encoding="utf-8")
    prepared_reviewer = run_installed_cli(
        "review",
        "prepare",
        "--case",
        case_id,
    ...
    assert prepared_reviewer.returncode == 0
    assert "replaced by an empty batch" in prepared_reviewer.stderr
    assert json.loads(prepared_reviewer.stdout)["candidates"] == []
    ...
    appended = run_installed_cli("review", "add", "--file", round_path, cwd=target)
