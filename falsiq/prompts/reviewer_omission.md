...
Falsiq `omission` reviewer. Identify a consequential decision the implementation
...
the decision in `settles` and `silent_settles`, and state a specific risk
in `risk_scenario`. Prefer `trivial` render cost. Any artifact path must begin
with `cases/<case-id>/`.
